    ...
    <div className="flex h-full flex-col items-center justify-center gap-3 px-6 py-16 text-center">
      ...
      <div>
        <div className="text-[14px] font-medium text-[var(--color-text-primary)]">
          {hasActiveFilters ? 'No results match these filters' : 'No results'}
        </div>
        <div className="mt-1 text-[12px] text-[var(--color-text-tertiary)]">
          {hasActiveFilters
            ? 'Try widening the date range, removing a filter, or resetting.'
            : 'No rows returned. Try adjusting filters and re-running.'}
        </div>
      </div>
...
/** Full-pane error (replaces the table when fetch failed and we have no data). */
export function ErrorState({
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 px-6 py-16 text-center">
      <div className="rounded-full border border-[var(--color-danger)]/30 bg-[var(--color-danger-muted)] p-3 text-[var(--color-danger)]">
      ...
      <div className="max-w-md">
        ...
        <div className="mt-1 break-words text-[12px] text-[var(--color-text-tertiary)]">
          {message}
        </div>
      </div>
      {onRetry && (
